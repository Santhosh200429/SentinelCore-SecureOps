...
export default function SecurityCommandCenterPage() {
  ...
  const [data, setData] = useState({ stats: null, assets: [], incidents: [], vulnerabilities: [], standards: [], alerts: [], events: [] });
  ...
  const load = useCallback(async (silent = false) => {
    ...
    const results = await Promise.allSettled([
      ...
      vulnerabilityService.getAll(), complianceService.getStandards(), alertService.getLive(), securityEventService.getAll(),
    ]);
    ...
    setData({
      ...
      vulnerabilities: list(value(3, [])), standards: list(value(4, [])), alerts: list(value(5, [])), events: list(value(6, [])),
    });
  ...
  const metrics = useMemo(() => {
    const { assets, incidents, vulnerabilities, standards, events } = data;
    ...
    return { criticalAssets, healthyAssets, activeIncidents: activeIncidents.length, criticalIncidents, criticalVulnerabilities, securityEvents: events.length, compliance, score: hasPostureData ? Math.max(0, Math.min(100, score)) : null };
  }, [data]);
  ...
  return <div className="scc-page">
    ...
    <section className="scc-top-grid">
      ...
      <Metric icon={<Activity/>} label="Security events" value={metrics.securityEvents} sub="recent endpoint evidence" tone="green"/>
    </section>
    ...
    <section className="scc-panels">
      ...
      <Panel title="Live security alerts" icon={<AlertTriangle/>} action={() => navigate('/security-events')}>
        {data.alerts.slice(0,6).map((alert,index) => <button className="scc-alert" key={alert.id ?? index} onClick={() => navigate('/security-events')}><span className={`alert-dot ${String(alert.severity || 'info').toLowerCase()}`}/><div><strong>{alert.title || 'Security alert'}</strong><small>{alert.source || 'SentinelCore'} · {alert.timestamp ? new Date(alert.timestamp).toLocaleString() : 'recent'}</small></div><ArrowUpRight size={15}/></button>)}
        {!data.alerts.length && <Empty text="No active alerts."/>}
      </Panel>
      <Panel title="Security event feed" icon={<Activity/>} action={() => navigate('/security-events')}>
        {data.events.slice(0,6).map((event,index) => <button className="scc-alert" key={event.id ?? index} onClick={() => navigate('/security-events')}><span className={`alert-dot ${String(event.severity || 'info').toLowerCase()}`}/><div><strong>{event.message || `Event ${event.eventId || index+1}`}</strong><small>{event.hostname || event.source || 'Endpoint'} · {event.timestamp ? new Date(event.timestamp).toLocaleString() : 'recent'}</small></div><ArrowUpRight size={15}/></button>)}
        {!data.events.length && <Empty text="No endpoint security events collected yet."/>}
      </Panel>
