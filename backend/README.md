...
5.  Sign in using the bootstrap credentials configured through `BOOTSTRAP_ADMIN_USERNAME` and `BOOTSTRAP_ADMIN_PASSWORD`.
## Real-Time Monitoring Mode

SentinelCore now supports a production-oriented live telemetry mode without fabricating operational metrics.

### Modes

```text
APP_DATA_MODE=live   # default; no seeded operational telemetry
APP_DATA_MODE=demo   # existing demo records for development/testing
```

### Live telemetry sources

- Windows host CPU, memory, disk, network, uptime and process data via OSHI
- JVM heap, threads, uptime and GC metrics via JVM management APIs
- PostgreSQL size, active connections and max connections via PostgreSQL statistics
- Real host asset discovery with a locally persisted machine identifier
- PostgreSQL-persisted telemetry snapshots
- Server-Sent Events (SSE) for live telemetry and security alerts
- Threshold-based alerts using actual telemetry only

### Endpoints

```text
GET  /api/infrastructure/telemetry
GET  /api/infrastructure/stream
GET  /api/infrastructure/asset
GET  /api/infrastructure/processes
GET  /api/alerts/live
GET  /api/alerts/stream
GET  /api/security-events
POST /api/security-events/collect-windows
POST /api/vulnerabilities/sync-nvd
```

If an integration is unavailable, the application reports `Data unavailable`, `Waiting for telemetry`, or `Integration not configured` instead of inventing values.
